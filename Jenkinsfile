pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "nginx-app:latest" // 이미지 이름 및 태그
    }
            ...
            steps {
                script {
                    docker.withRegistry("${DOCKER_REGISTRY}", DOCKER_CREDENTIALS_ID) {
                        //sh "docker tag ${IMAGE_NAME} ${DOCKER_REGISTRY}/web-server/${IMAGE_NAME}" // 이미지 태그 변경
                         sh "docker push ${DOCKER_Harbor_path}/web-server/${IMAGE_NAME}" // 이미지를 Docker Harbor에 푸시
                    }
                }
            }
        }
